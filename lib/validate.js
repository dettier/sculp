...
import { isArray, isString, isFunction, keys } from 'lodash-compat';
...
const debug = require('debug')('schemer:validate');
...
function calculateAndValidateRule (scheme, ruleName,
  ...
  if (ruleName === CUSTOM_RULE_NAME) {
    ...
    const nameWithoutPrefix = ruleName.slice(VALIDATIONS_PROPERTY_NAME_PREFIX.length);
    validation = VALIDATIONS[nameWithoutPrefix];
  ...
  if (isFunction(validation) === false) {
    if (typeof console !== 'undefined') {
      ...
      console && console.warn(`Unknown validation ${ruleName}`);
    }
    return res;
  }
...
export const validateField = (value, scheme, path, context) => {
  ...
  // fromCache flag
  //////////////////////////////////////////////////////////////////////////////
  // we need to set fromCache flag to false
  ...
  //if (shouldBeRemovedAsEmpty(res, scheme, path))
  //  res = undefined;
...
export default (rootValue, rootScheme, path = '', options = {}) => {
  ...
  const result = validateField(_value, _scheme, path, context);

  return { result, fromCache : context.fromCache };
};
